/**
 * Freeflow - User-facing copy cleanup.
 * Keeps internal/vendor messages hidden and shows simple English support text.
 */
function stripInternalBranding(value) {
  if (value == null) return value;

  var brand = typeof EXTENSION_NAME !== "undefined" ? String(EXTENSION_NAME) : "Freeflow";
  var s = String(value);
  ...
  var rules = [
    ...
    [/on\s+supabase/gi, ""],
    [/admin\s*→[^.]*\.?/gi, ""],
    [/check\s+admin[^.]*\.?/gi, "Email Admin - scoopwithpiper@gmail.com"],
    ...
    [/discord\s+support/gi, "admin support"],
    [/discord/gi, "admin support"],
    [/\s{2,}/g, " "],
    [/\. \./g, "."],
    [/\s+\./g, "."],
    [/^\s+|\s+$/g, ""]
...
function translateUserMessage(value) {
  ...
  var map = [
    [/Licen[çc]a\s+n[aã]o\s+encontrada\s+ou\s+inativa/ig,
      "License could not be validated. Check your key or Email Admin - scoopwithpiper@gmail.com"],
    ...
    [/Sess[aã]o\s+inv[aá]lida\.?\s*Fa[çc]a\s+login\s+novamente\.?/ig, "Invalid session. Please activate again."],
    [/Sess[aã]o\s+inv[aá]lida/ig, "Invalid session"],
    [/Fa[çc]a\s+login\s+novamente/ig, "Please activate again"],
    ...
    [/Nenhum\s+QL\s+configurado/ig, "Service is temporarily unavailable. Email Admin - scoopwithpiper@gmail.com"],
    [/No\s+vendor\s+license\s+configured[^.]*/ig, "Service is temporarily unavailable. Email Admin - scoopwithpiper@gmail.com"],
    [/Vendor\s+license\s+not\s+found[^.]*/ig, "License could not be validated. Email Admin - scoopwithpiper@gmail.com"],
