...
function templateLicenseGate(minimized) {
  return '<div id="ql-header">' +
    '<div class="ql-header-left">' +
      ...
      '<img class="ql-title-logo" src="' + chrome.runtime.getURL('assets/logo-master-lovable-square.png') + '" alt=""><span class="ql-title">Freeflow</span>' +
    '</div>' +
  ...
  '<div id="ql-body">' +
    '<div class="ql-license-gate">' +
      ...
      '<p class="ql-gate-title">License Key Login</p>' +
      '<p class="ql-gate-desc">Enter your license key to unlock Freeflow. Email Admin - scoopwithpiper@gmail.com</p>' +
      '<div class="ql-field">' +
        '<input id="ql-license-input" placeholder="Enter license key" spellcheck="false">' +
      '</div>' +
      '<button id="ql-validate-btn">Login with License</button>' +
      '<div id="ql-license-log"></div>' +
      '<div class="ql-contact-admin">Email Admin - scoopwithpiper@gmail.com</div>' +
    '</div>' +
  '</div>' +
...
function templateMainUI(greeting, statusBadge, minimized) {
  return '<div id="ql-header">' +
    '<div class="ql-header-left">' +
      '<span class="ql-brand"><img class="ql-brand-logo" src="' + chrome.runtime.getURL('assets/logo-master-lovable-square.png') + '" alt=""><span>Freeflow</span></span>' +
    '</div>' +
    '<div class="ql-header-right">' +
  ...
  '<div id="ql-footer" class="ql-footer">' +
    ...
    '</div>' +
    '<span class="ql-badge-mz">Freeflow</span>' +
    '<div class="ql-contact-admin">Email Admin - scoopwithpiper@gmail.com</div>' +
  '</div>' +
  '<div id="ql-resize-handle" class="ql-resize-handle"></div>' +
  '<!-- Notifications Panel -->' +
...
function templateExpiredOverlay() {
  return '<div class="ql-sweetalert-box">' +
    ...
    '<p class="ql-sweetalert-text">Your license has expired. Email Admin - scoopwithpiper@gmail.com</p>' +
    '<div class="ql-sweetalert-actions">' +
      '<button class="ql-sweetalert-btn ql-sweetalert-btn-primary" id="ql-sweetalert-renew">💬 Email Admin - scoopwithpiper@gmail.com</button>' +
...
var QL_BRL_PLANS = [
  { name: "Weekly",  price: "49,90",  period: "per week",      popular: false, badge: "7d",  icon: "⚡",
    features: ["Full extension access", "Plan Mode workflow", "Admin support"] },
...
function templateBrlCard(plan, idx) {
  ...
  return '<div class="ql-pkg-card ql-pkg-brl' + (plan.popular ? ' ql-pkg-highlight' : '') + '" data-brl-idx="' + idx + '">' +
    ...
    '<button class="ql-pkg-select-btn ql-brl-buy">💬 Email Admin - scoopwithpiper@gmail.com</button>' +
  '</div>';
}
function templateBrlSection() {
  var cards = QL_BRL_PLANS.map(function(p, i){ return templateBrlCard(p, i); }).join('');
  return '<div class="ql-pay-divider"><span>Admin support</span></div>' +
...
function templatePaymentUI(minimized) {
  ...
  '<div id="ql-body">' +
    '<div class="ql-pay-section">' +
      '<div class="ql-pay-title">Open Admin support</div>' +
      templateBrlSection() +
      '<div class="ql-pay-divider"><span>Admin support</span></div>' +
      '<div id="ql-packages-list" class="ql-packages-list">' +
        '<div class="ql-pay-loading">⏳ Email Admin - scoopwithpiper@gmail.com</div>' +
      '</div>' +
    '</div>' +
  '</div>' +
  '<div id="ql-resize-handle" class="ql-resize-handle"></div>';
...
function templatePackageCard(pkg) {
  ...
  return '<div class="ql-pkg-card' + (pkg.is_popular ? ' ql-pkg-highlight' : '') + '" data-pkg-id="' + escapeHtml(pkg.id) + '" data-pkg-name="' + escapeHtml(pkg.name) + '" data-pkg-price="' + '' + '">' +
    ...
    '<ul class="ql-pkg-features">' + features + '</ul>' +
    '<button class="ql-pkg-select-btn">Email Admin - scoopwithpiper@gmail.com</button>' +
  '</div>';
}
...
function templateCheckoutScreen(pkg, minimized) {
  ...
  '<div id="ql-body">' +
    '<div class="ql-pay-section" style="text-align:center">' +
      '<div class="ql-pay-title">Request your license key from admin</div>' +
      '<div class="ql-selected-pkg">📦 <strong>' + escapeHtml(pkg.name || 'Selected plan') + '</strong></div>' +
      '<p style="color:var(--ql-text-secondary);font-size:12px;line-height:1.5;margin:10px 0 16px">Support and license purchases are handled through admin support.</p>' +
      '<a href="' + QL_DISCORD_SUPPORT + '" target="_blank" rel="noopener noreferrer" class="ql-confirm-pay-btn" style="display:block;text-decoration:none;margin-bottom:10px">💬 Email Admin - scoopwithpiper@gmail.com</a>' +
      '<button id="ql-confirm-pay" class="ql-buy-btn" style="font-size:12px;width:100%">Email Admin - scoopwithpiper@gmail.com</button>' +
...
function templatePaymentSuccess(licenseKey) {
  return '<div class="ql-pay-section" style="text-align:center;padding:24px 16px">' +
    ...
    '<div class="ql-pay-title">Admin Support Confirmed!</div>' +
