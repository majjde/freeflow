// ============================================
// Empire – Business Logic (content)
// HTML templates are in content-templates.js
// ============================================

console.log("[ContentScript] Empire loaded");
...
const LICENSE_BASE = typeof LICENSE_API_BASE !== "undefined" ? LICENSE_API_BASE : API_BASE;
const LICENSE_KEY = typeof LICENSE_API_KEY !== "undefined" ? LICENSE_API_KEY : API_KEY;
...
function licenseApiHeaders(extra) {
  return Object.assign({ apikey: LICENSE_KEY }, extra || {});
}
...
function activateInternalSession() {
  return bgFetch(VALIDATE_URL, {
    method: "POST",
    headers: licenseApiHeaders({ "Content-Type": "application/json" }),
    body: JSON.stringify({
...
function setupWhatsAppPopup() {
  ...
  const joinBtn = document.getElementById("ql-wa-join");
...
function injectShieldOverlay(){
  ...
  overlay.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' +
    ...
    '</svg>' +
    '<span class="ql-shield-overlay-text">🛡️ Protected by Empire</span>' +
...
async function showPaymentUI(box, preselectedPkg){
  ...
  document.querySelectorAll(".ql-brl-buy").forEach(function(btn){
    btn.addEventListener("click", function(){
      ...
      if(!plan) return;
      var msg = "Hello! 👋 I am interested in the *" + plan.name + "* plan for Empire Lovable Vip Extension (R$ " + plan.price + " - " + plan.period + ").\n\nOpen support for more information.";
      var url = DISCORD_URL;
  ...
  try {
    ...
    if(!packages || !Array.isArray(packages) || packages.length === 0){
      list.innerHTML = '<div class="ql-pay-loading">Open Admin Support</div>';
      return;
    }
    ...
    const list = document.getElementById("ql-packages-list");
    if(list) list.innerHTML = '<div class="ql-pay-loading">Open Admin Support</div>';
  }
}
  ...
  if(confirmBtn){
    confirmBtn.addEventListener("click", async () => {
      ...
      if(!phoneInput){
        ...
        if(log){ log.className = "ql-pay-log ql-pay-info"; log.textContent = "Opening Admin support..."; }
        return;
      }
      ...
      try {
        ...
        if(result && result.status === "sucesso"){
          ...
          const errMsg = (result && result.error) ? result.error : "Admin Support failed. Please try again.";
          if(log){ log.className = "ql-pay-log ql-pay-error"; log.textContent = "✗ " + errMsg; }
          confirmBtn.disabled = false;
          confirmBtn.textContent = "Open Admin Support";
        }
      } catch(err) {
...
function injectNativeChatOverlay() {
  ...
  if (!document.getElementById("ql-native-badge")) {
    ...
    badge.innerHTML = "⚡ <span>Freeflow</span>";
