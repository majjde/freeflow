// ============================================================
// Empire - Side Panel Logic (Business Logic Only)
...
(function(){
  ...
  function clearInternalFreeLicenseIfNeeded(res) {
    res = res || {};
    var key = String(res.ql_license_key || "").trim().toUpperCase();
    var status = String(res.ql_license_status || "").trim().toLowerCase();
    if (key === "INTERNAL" || status === "internal" || status === "free") {
      return new Promise(function(resolve) {
        try {
  ...
  (async function init() {
    ...
    try { chrome.storage.local.set({ ql_profile_device_id: deviceId }); } catch(e) {}
    ...
    chrome.storage.local.get(["ql_license_valid","ql_license_key","ql_user_name","ql_expires_at","ql_activated_at","ql_license_status","ql_validity_minutes","ql_session_id"], async (res) => {
      res = await clearInternalFreeLicenseIfNeeded(res);
      ...
      // Force WhatsApp popup when side panel is opened
      if (typeof setupWhatsAppPopup === "function") {
        setupWhatsAppPopup();
      } else if (typeof window.parent.setupWhatsAppPopup === "function") {
        window.parent.setupWhatsAppPopup();
      }
    });
  })();
